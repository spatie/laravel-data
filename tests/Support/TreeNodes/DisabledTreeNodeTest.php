...
it('can merge a node', function () {
    $node = new DisabledTreeNode();

    expect($node->merge(new AllTreeNode()))->toEqual(new AllTreeNode())
        ->and($node->merge(new ExcludedTreeNode()))->toEqual(new ExcludedTreeNode())
        ->and($node->merge(new DisabledTreeNode()))->toEqual(new DisabledTreeNode())
        ->and($node->merge(new PartialTreeNode()))->toEqual(new PartialTreeNode())
        ->and(
