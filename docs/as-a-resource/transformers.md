...
It is possible to get an array representation of a data object without transforming the properties. This means `Carbon` objects won't be transformed into strings. And also, nested data objects, `DataCollection`s won't be transformed into arrays and the names of the properties won't be change to `#[MapOutputName('diffrent_name')]` . You can do this by calling the `all` method on a data object like this:
