...
{
    public static function multi(ReflectionProperty $property, int $count)
    {
        return new self("A data property can only have one type, {$property->class}::{$property->name} has {$count} types");
    }

    public static function lazy(ReflectionProperty $property, int $count)
    {
    ...
    public static function nullableLazy(ReflectionProperty $property, int $count)
    {
